...
where_to_start_list = ["fq", "fastq_sort", "fastqc", "trimming", "sam", "second_fastqc",
                      ...
                      "vcf_filter", "ann_vep", "ann_inter", "full_ann", "panel_ann"]
...
do_until_list = ["fq", "fastq_sort", "fastqc", "trimming", "sam", "second_fastqc",
                      "bam", "bam_sort", "bam_dup", "bam_index", "bam_merge", "coverage", "vcf",
                      "vcf_filter", "ann_vep", "ann_inter", "full_ann", "panel_ann", "full"]
    ...
    if dir_with_fq.split("/")[-1] in ["lane_1", "lane_2", "lane_3", "lane_4"]:
        ...
        if where_to_start in ["fq", "trimming"]:
            process = subprocess.check_call(["java", "-jar",
                                             ...
                                             "SE", "-threads", "2", dir_with_fq + "/" + file1,
                                             dir_with_fq + "/trimmed" + file1,
                                             "HEADCROP:3"])
            if input_file_types == "PE fq":
                process = subprocess.check_call(["java", "-jar",
                                                 "/data/programs/trimmomatic/Trimmomatic-0.39/trimmomatic-0.39.jar",
                                                 "SE", "-threads", "2", dir_with_fq + "/" + file2,
        ...
        if where_to_start in ["fq", "trimming", "fastqc", "sam"]:
            os.system("bwa mem " +
                      ...
                      dir_with_fq + "/trimmed" + file1 + " " + dir_with_fq + "/trimmed" + file2 + " -t 2 > " + dir_with_fq + "/" + sample_name + ".sam")
            if do_until == "sam":
                sys.exit("sam done")
...
if where_to_start in ["fq", "fastq_sort", "fastqc", "trimming", "sam", "second_fastqc",
    ...
    # add panel ann here
    process = subprocess.check_call(["python3", "/data/CGT_RSMU/scripts/filter_gene2.py",
                                     'ANNOT_' + sample_name + '.xlsx',
                                     '/data/ref_and_beds/Kholestazy_spisok_1.xlsx',
                                     'gene_panel_' + sample_name + '.xlsx'])

    print(str(datetime.now() - startTime) + " for panel_ann level")
    if do_until == "panel_ann":
        sys.exit("panel_ann done")
